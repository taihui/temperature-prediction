...
def rnn_model(lookback_hours, delay_hours, batch_size, num_epoch, model_path):
    ...
    ######## Build RNN model ########
    model = Sequential()
    model.add(SimpleRNN(256, input_shape=(input_train_X.shape[1], input_train_X.shape[2]),
                        return_sequences=True,dropout=0.1,recurrent_dropout=0.5))
    model.add(SimpleRNN(512,return_sequences=True, dropout=0.1,recurrent_dropout=0.5))
    model.add(SimpleRNN(256, activation='elu', dropout=0.1,recurrent_dropout=0.5))
    #
    model.add(Dense(1))
    ...
    plt.ylabel('MAE')
    plt.legend()
    plt.savefig('figure/simpleRNN_Train_MAE_'+ str(lookback_hours) + '_' + str(delay_hours) + '.png')
